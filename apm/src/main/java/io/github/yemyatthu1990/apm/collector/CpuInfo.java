...
public class CpuInfo {
     ...
     */
    static double getCpuUsageSinceLastCall() {
     ...
     */
     static double getCpuUsage(double[] coresUsage) {
        ...
        return (double)cpuUsage / (coresUsage.length - 1);
    }
     ...
     */
    static synchronized double[] getCoresUsageGuessFromFreq()
    {
        initCoresFreq();
        int nbCores = mCoresFreq.size() + 1;
        double[] coresUsage = new double[nbCores];
        coresUsage[0] = 0;
        ...
        if (mCoresFreq.size() > 0)
            coresUsage[0] /= (double)mCoresFreq.size();
        return coresUsage;
    }

    static void initCoresFreq()
    {
        if (mCoresFreq == null) {
...
//                else
                cpuUsage = (double)(cur - min) * 100 / (max - min);
            }
            return cpuUsage;
        }
    }
     ...
     */
    static synchronized double[] getCoresUsageDeprecated() {
        ...
        double[] coresUsage = new double[numCores];
